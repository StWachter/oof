...
class RedactorInputRender extends modTemplateVarInputRender {
    ...
    public function render($value, array $params = array()) {
        ...
        foreach ($params as $key => $v) {
            if (($value === 'inherit' || $v === '') && isset($systemOptions[$key])) {
        ...
        // Re-create the plugins array to make sure things can be disabled
        $params['plugins'] = array();
        $params['plugins'][] = 'modmore';
        ...
        if ($params['buttonFullScreen']) {
            $params['plugins'][] = "fullscreen";
        } else {
            $params['plugins'] = array_diff($params['plugins'], array('fullscreen'));
        }
        ...
        if (in_array('replacer', $params['plugins'], true) && $this->redactor->getBooleanOption('redactor.plugin_replacer_button')) {
            $options['replacerButton'] = true;
        }
...
HERE;
    ...
    protected function registerStuff() {
        $this->modx->controller->addCSS($this->redactor->config['assetsUrl'].'redactor-2.3.7.min.css');
        ...
        $this->modx->controller->addJavascript($this->redactor->config['assetsUrl'].'redactor-2.3.7.min.js');
$script = <<<HERE
<script>try {ace} catch(e) { document.write('<script src="https://cdnjs.cloudflare.com/ajax/libs/ace/1.1.9/ace.js"><\/script>') }</script>
HERE;
