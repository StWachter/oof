...
class Redactor {
    ...
    */
    ...
    function __construct(modX &$modx,array $config = array()) {
        ...
        $this->version = new VersionObject(2, 3, 8, 'pl');
...
HERE;
        ...
        if (in_array('replacer', $plugins, true) && $this->getBooleanOption('redactor.plugin_replacer_button')) {
...
Ext.onReady(function() {
    ...
    public function initialize() {
        if (!$this->assetsLoaded) {
            ...
            $this->modx->controller->addCSS($this->config['assetsUrl'].'redactor-2.3.7.min.css');
            ...
            $this->modx->controller->addJavascript($this->config['assetsUrl'].'redactor-2.3.7.min.js');
    ...
    */
    ...
    public function getCustomStyles() {
        ...
        foreach($styles as $style) {
            $classClass = implode('.', explode(' ', $style['class']));
            $className = '.redactor-dropdown-' . $style['tag'] . '-' . $classClass;
